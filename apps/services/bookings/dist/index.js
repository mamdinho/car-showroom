...
import { DynamoDBDocumentClient, GetCommand, UpdateCommand, QueryCommand, TransactWriteCommand, DeleteCommand } from "@aws-sdk/lib-dynamodb";
...
async function listMyBookings(event) {
    const { userId } = getAuthContext(event);
    if (!userId)
        return forb();
...
async function updateBooking(event) {
    ...
    const item = get.Item;
    if (!item || item.type === "lock")
        return notf();
    // Authorization rules:
    // - admin can set any status
    // - owner can set status to "cancelled" only
    if (!isAdmin && !(item.userId === userId && status === "cancelled"))
        return forb();
    // Update status
    ...
    if (status === "cancelled") {
        ...
        try {
            await ddb.send(new DeleteCommand({ TableName: BOOKINGS_TABLE, Key: { bookingId: lockId } }));
        }
        catch { }
    }
